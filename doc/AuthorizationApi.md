...
final api = K8s().getAuthorizationApi();

try {
    final response = api.getAuthorizationAPIGroup();
    print(response);
} catch on DioError (e) {
    print('Exception when calling AuthorizationApi->getAuthorizationAPIGroup: $e\n');
}
```
