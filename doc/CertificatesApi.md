...
final api = K8s().getCertificatesApi();

try {
    final response = api.getCertificatesAPIGroup();
    print(response);
} catch on DioError (e) {
    print('Exception when calling CertificatesApi->getCertificatesAPIGroup: $e\n');
}
```
