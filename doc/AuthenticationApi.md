...
final api = K8s().getAuthenticationApi();

try {
    final response = api.getAuthenticationAPIGroup();
    print(response);
} catch on DioError (e) {
    print('Exception when calling AuthenticationApi->getAuthenticationAPIGroup: $e\n');
}
```
