...
class IoK8sApiCoreV1SELinuxOptions {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is IoK8sApiCoreV1SELinuxOptions &&
     other.level == level &&
     other.role == role &&
     other.type == type &&
     other.user == user;
  ...
  @override
  String toString() => 'IoK8sApiCoreV1SELinuxOptions[level=$level, role=$role, type=$type, user=$user]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "IoK8sApiCoreV1SELinuxOptions[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "IoK8sApiCoreV1SELinuxOptions[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<IoK8sApiCoreV1SELinuxOptions>? listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<IoK8sApiCoreV1SELinuxOptions>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<IoK8sApiCoreV1SELinuxOptions>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = IoK8sApiCoreV1SELinuxOptions.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
