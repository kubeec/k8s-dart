...
class IoK8sApiCoreV1Event {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is IoK8sApiCoreV1Event &&
     other.action == action &&
  ...
  @override
  String toString() => 'IoK8sApiCoreV1Event[action=$action, apiVersion=$apiVersion, count=$count, eventTime=$eventTime, firstTimestamp=$firstTimestamp, involvedObject=$involvedObject, kind=$kind, lastTimestamp=$lastTimestamp, message=$message, metadata=$metadata, reason=$reason, related=$related, reportingComponent=$reportingComponent, reportingInstance=$reportingInstance, series=$series, source_=$source_, type=$type]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "IoK8sApiCoreV1Event[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "IoK8sApiCoreV1Event[$key]" has a null value in JSON.');
        });
        return true;
      }());
      ...
      return IoK8sApiCoreV1Event(
        ...
        involvedObject: IoK8sApiCoreV1ObjectReference.fromJson(json[r'involvedObject'])!,
  ...
  static List<IoK8sApiCoreV1Event>? listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<IoK8sApiCoreV1Event>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<IoK8sApiCoreV1Event>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = IoK8sApiCoreV1Event.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
