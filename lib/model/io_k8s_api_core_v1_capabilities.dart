...
class IoK8sApiCoreV1Capabilities {
  ...
  static IoK8sApiCoreV1Capabilities? fromJson(dynamic value) {
    if (value is Map) {
      ...
      return IoK8sApiCoreV1Capabilities(
        add: json[r'add'] is List
            ? (json[r'add'] as List).cast<String>()
            : const [],
        drop: json[r'drop'] is List
            ? (json[r'drop'] as List).cast<String>()
            : const [],
      );
    }
    return null;
  ...
  static Map<String, List<IoK8sApiCoreV1Capabilities>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = IoK8sApiCoreV1Capabilities.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
